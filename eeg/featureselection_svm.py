...
train_data = np.genfromtxt(main_path+"train-"+feature_type+"-eeg-x.csv",delimiter=',')
train_target =  np.genfromtxt(main_path+"train-"+feature_type+"-eeg-y.csv",delimiter=',')
test_data = np.genfromtxt(main_path+"test-"+feature_type+"-eeg-x.csv",delimiter=',')
test_target =  np.genfromtxt(main_path+"test-"+feature_type+"-eeg-y.csv",delimiter=',')
...
sfs = SequentialFeatureSelector(svmModel,
          k_features=35,
          forward=True,
          floating=False,
          scoring='accuracy',
...
while test_n > 0:
    ...
    for train, test in kFold.split(train_data, train_target):
        i+=1
        svmModel.fit(train_data[train], train_target[train])
        y_pred = svmModel.predict(train_data[test])
...
writeExcel(ac,"results-svm-"+feature_type)